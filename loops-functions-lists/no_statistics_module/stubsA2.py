...
#To compute the mean and standard deviation is necessary to go through the netIncomeList with a for iteration and adding up
#the corresponding sums in each iteration. At the end this functions will return the mean and standard deviation value of all
#the net incomes.
...
def mean( inputList ) : #Step 3
    ...
    for i in inputList:
        result+=i
    return result/numberOfEmployees
    
def standardDeviation( inputList, mean ) : #Step 4
    result=0
    for i in inputList:
        result+=(i-mean)**2
    return sqrt(result/numberOfEmployees)